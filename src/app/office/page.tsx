  ...
  async function handleChat() {
    ...
    try {
      const res = await fetch("/api/chat", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ message: msg, channelId, walletAddress: publicKey.toBase58() }),
      });
      ...
      if (res.status === 402) {
        setThinking(null);
        setChat((p) => [...p, { role: "nova", text: `Insufficient credits. Balance: ${(data.balance / 1_000_000).toFixed(4)} USDC. Load credits to continue.` }]);
        return;
      }
      ...
      if (data.delegatedTo) {
        ...
        if (agent) {
          ...
          if (data.delegatedResponse) {
            ...
            if (agentData.balance !== undefined) setBalance(agentData.balance);
          }
        }
      }
    } catch {
